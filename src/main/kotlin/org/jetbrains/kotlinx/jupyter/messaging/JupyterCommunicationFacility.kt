...
fun JupyterCommunicationFacility.sendResponse(response: Response, requestCount: Long, startedTime: String) {
    if (!response.stdOut.isNullOrEmpty()) {
        sendOut(JupyterOutType.STDOUT, response.stdOut)
    }
    if (!response.stdErr.isNullOrEmpty()) {
        sendOut(JupyterOutType.STDERR, response.stdErr)
    }
