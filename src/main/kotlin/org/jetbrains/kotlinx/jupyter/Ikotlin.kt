...
private fun parseCommandLine(vararg args: String): KernelArgs {
    ...
    var replMode: ReplMode = KERNEL_DEFAULT_REPL_MODE
    args.forEach { arg ->
        when {
            ...
            arg.startsWith("-replMode=") -> {
                replMode = ReplMode.entries.firstOrNull {
                    it.name ==
                        arg.substringAfter(
                            '=',
                        )
                } ?: throw IllegalArgumentException("Invalid replMode: $arg")
            }
            else -> {
    ...
    return KernelArgs(cfgFileValue, classpath ?: emptyList(), homeDir, debugPort, clientType, jvmTargetForSnippets, replMode)
}
