...
class CompilerServiceImpl(
    ...
    private val currentImports: MutableList<String> = mutableListOf()
    private var lastDeclarations: List<DeclarationInfo>? = null

    private val scriptDataCollectors: List<ScriptDataCollector> =
        listOf(
            ImportsCollector { currentImports += it },
    ...
    private fun onAnnotationsHandler(context: ScriptConfigurationRefinementContext): ResultWithDiagnostics<ScriptCompilationConfiguration> {
        ...
        for (annotationData in annotations) {
            when (val annotation = annotationData.annotation) {
                ...
                is Repository -> {
                    dependencyAnnotations.add(DependencyAnnotation.Repository(annotation.value, annotation.username, annotation.password))
                }
                is CompilerArgs -> {
    ...
    override suspend fun getCompilerData(): CompilerData =
        CompilerData(
            KotlinCompilerVersion.VERSION,
            compilationConfig[ScriptCompilationConfiguration.fileExtension]!!,
        )
    ...
    ): List<SourceCodeCompletionVariant> = compiler.complete(code, position, id)
...
private class ImportsCollector(
    private val reportImports: (List<String>) -> Unit,
) : ScriptDataCollector {
...
private class DeclarationsCollector(
    private val reportDeclarations: (List<DeclarationInfo>) -> Unit,
) : ScriptDataCollector {
    override fun collect(scriptInfo: ScriptDataCollector.ScriptInfo) {
        ...
        reportDeclarations(declarations)
    }
}
