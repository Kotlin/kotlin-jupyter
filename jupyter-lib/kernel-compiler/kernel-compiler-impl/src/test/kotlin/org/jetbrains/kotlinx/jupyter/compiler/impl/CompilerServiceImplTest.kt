 ...
 *
 * These tests verify that compiled snippets can be serialized for transmission over RPC.
...
class CompilerServiceImplTest {
    ...
    private val kotlinStdlibClasspath =
        System
    ...
    @Test
    fun `should serialize compiled snippet successfully`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `should serialize snippet with multiple statements`() =
        runBlocking<Unit> {
            val compiler = createCompilerService()
    ...
    @Test
    fun `should handle compilation errors without serialization`() =
        runBlocking {
            val compiler = createCompilerService()
    ...
    @Test
    fun `serialized snippets should be reusable across multiple compilations`() =
        runBlocking<Unit> {
            val compiler = createCompilerService()
    ...
    @Test
    fun `should populate cache when deserializing with cache parameter`() =
        runBlocking {
            val compiler = createCompilerService()
