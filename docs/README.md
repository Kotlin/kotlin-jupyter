...
replace the `{VERSION}` parameter with the desired PyPi version of the Kotlin Jupyter Kernel (for example, `0.14.1.505`).
...
`conda install -c jetbrains kotlin-jupyter-kernel={0.14.1.505}`
...
`pip install kotlin-jupyter-kernel=={0.14.1.505} --ignore-installed`
...
2. Replace the `{VERSION}` parameter for the latest PyPi version of the Kotlin Jupyter Kernel (for example, `0.14.1.505`).
