...
The Kotlin Kernel for Jupyter notebooks is a powerful tool that allows you to write and run [Kotlin](https://kotlinlang.org/) 2.2.20-dev-7701 code within the
...
The latest version of the Kotlin Kernel for notebooks uses the Kotlin compiler of version 2.2.20-dev-7701.
...
replace the `{VERSION}` parameter with the desired PyPi version of the Kotlin Jupyter Kernel (for example, `0.14.1.506`).
...
`conda install -c jetbrains kotlin-jupyter-kernel={0.14.1.506}`
...
`pip install kotlin-jupyter-kernel=={0.14.1.506} --ignore-installed`
...
2. Replace the `{VERSION}` parameter for the latest PyPi version of the Kotlin Jupyter Kernel (for example, `0.14.1.506`).
